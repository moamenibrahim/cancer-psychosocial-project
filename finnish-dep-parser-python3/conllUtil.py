...
def printc(cols,out=sys.stdout):
    ...
    print((u"\t".join(cols)).encode("utf-8"), file=out)
...
def dropNullsSent(sentenceCounter,currSentence):
    ...
    for idx,cols in enumerate(currSentence):
        cols[c2i["ID"]]=str(int(cols[c2i["ID"]])-corrections[idx])
        if cols[c2i["HEAD"]] not in (u"_",u"0"):
            ...
            if head in nulls:
                ...
                cols[c2i["HEAD"]]=str(head-corrections[head-1])
        if cols[c2i["PHEAD"]] not in (u"_",u"0"):
            ...
            if phead in nulls:
                ...
                cols[c2i["PHEAD"]]=str(phead-corrections[phead-1])
...
if __name__=="__main__":
    ...
    if options.action=="cat":
        ...
        for p in pairs:
            ...
            if result=="None":
                result=None
            exp=re.compile(str(regexp,"utf-8"),re.U)
        ...
        for (mainLine,) in readFiles(args):
            ...
            if cols[c2i["PFEAT"]]!=u"_":
                cols[c2i["PFEAT"]]=list(filter(cols[c2i["PFEAT"]],rewrites))
            if cols[c2i["FEAT"]]!=u"_":
                cols[c2i["FEAT"]]=list(filter(cols[c2i["FEAT"]],rewrites))
            printc(cols)
