...
if __name__=="__main__":
    ...
    if options.train:
        for line in sys.stdin:
                ...
                if feat!=u"_":
                    for cat_tag in feat.split(u"|"):
                        ...
                        if cat not in omor.cat2idx:
                            print("Unknown cat:", cat, file=sys.stderr)
                        else:
            ...
            if not s:
                s=u"_"
            print((str(idx-1)+u"\t"+token+u"\t"+pos+u"\t"+s+u"\t"+marmot_feats).encode("utf-8"))
        ...
        for line in sys.stdin:
            ...
            if len(cols)==1:
                ...
                if not marmot_feats:
                    marmot_feats=u"_"
                print(cols[1]+u"\t"+marmot_feats, file=f) #wordform tab feats
        f.close()
        #Now invoke marmot
        try:
...
#                    plemma,ppos,pfeat=txt,u"Num",u"_"
        ...
        #                    ptaglist[omor.cat2idx[u"SUBCAT"]]=u"Prop"
                        ...
                        #soft
                        ppos=pred[5]
                        pfeat=re.sub(r"((^|\|)[A-Z]+)_([a-zA-Z0-9])",r"\1=\3",pred[7],re.U)
                if len(inCols)==10:
