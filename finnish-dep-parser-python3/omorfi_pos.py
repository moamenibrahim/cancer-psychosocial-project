...
['POS', 'SUBCAT', 'NUM', 'CASE', 'POSS', 'PRS', 'VOICE', 'TENSE', 'MOOD', 'NEG', 'PCP', 'INF', 'CLIT', 'DRV', 'CMP', 'CASECHANGE', 'OTHER']
...
print("Loading Omorfi transducer in Java-as-subprocess inside Python implementation", file=sys.stderr)
...
cat_list=['POS', 'SUBCAT', 'NUM', 'CASE', 'POSS', 'PRS', 'VOICE', 'TENSE', 'MOOD', 'NEG', 'PCP', 'INF', 'CLIT', 'DRV', 'CMP', 'CASECHANGE', 'OTHER']

cat2idx={} #{category:index in cat_list}
for idx,cat in enumerate(cat_list):
    cat2idx[cat]=idx

for cat_tag in raw_tag_information.split(","):
...
for line in additionalReadingDefs.split("\n"):
    line=line.strip()
    if not line:
        continue
    token,r=line.split()
...
def _raw_analyze_taglist(tags):
    ...
    if isinstance(tags,str):
        tags=str(tags,"utf-8") #enforce unicode, wherever you are
    if isinstance(tags,str):
    ...
    for cat,tags in list(res.items()):
        if cat in ("CLIT","DRV","OTHER") and len(tags)>1:
            tags[:]=["+".join(tags)]
        assert len(tags)==1, (cat,tags)
        res[cat]=tags[0] #turn the lists into strings, so we have one value per category
    if "POS" not in res:
        subcat=res.get("SUBCAT",None)
        if subcat in ("CC","CS"):
            res["POS"]="C" #Omorfi fails to generate POS for CC&CS
        elif subcat in ("Para",):
            res["POS"]="Punct"
    if not "POS" in res:
        #happens with sun and sunkin at least, maybe others?
        raise ValueError(("There is no POS among %s, doesn't look like a valid reading."%("".join(tags))).encode("utf-8"))
    return res
...
def analyze_taglist(tags,retForm=RET_LIST):
    ...
    if retForm==RET_DICT:
        ...
        pos=ct_dict["POS"]
        tagString="|".join(cat+":"+ct_dict[cat] for cat in cat_list if cat!="POS" and cat in ct_dict)
...
def analyze_reading(reading):
    ...
    if reading.startswith("+<"):
        ...
        compoundParts=reading.split("+") #TODO: what if "+" is part of the token? ;)
    lemma=[]
    for compoundPart in compoundParts:
        if compoundPart.startswith("#") and len(compoundPart)>1 and compoundPart!="#<Punct>":
    ...
    for idx in range(len(tags)-1,-1,-1):
        ...
        if cat=="POS":
            ...
            if foundPOS: #looks like I'm done here!
                ...
                if tags[idx] not in ("Der_","Der_inen"): #checked these and they're okay
                    print(tags[idx], "is a deriv before POS in", reading.encode("utf-8"), file=sys.stderr)
        elif foundPOS:
            print(tags[idx], "is a tag ahead of POS in", reading.encode("utf-8"), file=sys.stderr)
    tags=tags[idx:]
    return lemma, tags
...
def omorfi_tag_token(token): #Token() from tree
    ...
    for lemma,tags,whole_reading in readings:
        try:
            ...
            print("Odd analysis, skipping:", whole_reading.encode("utf-8"), file=sys.stderr)
            continue
        token.posTags.append([None,lemma," ".join(t for t in tags if t),taglist])
    #Done
...
capRe=re.compile(r"<(Cap|cap|CAP)>",re.U) #The @....@ tags in the transducer output, want to get rid of them
pxRe=re.compile(r"<Px(Sg|Pl)3>",re.U)
...
def omorfi_postprocess(token,readings):
    ...
    #Pcle2Adv
    readings=[r.replace("Pcle","Adv") for r in readings]
    #Px3
    readings=[pxRe.sub("<Px3>",r) for r in readings]
    readings=[capRe.sub("",r) for r in readings] #Remove cap if any
    if is_up(token):
        readings=[r+"<Up>" for r in readings]

    if token.lower() in olla_pl1: # omorfi says 'olemme' and 'olette' are pl3
        readings=[r.replace("<Pl3>","<Pl1>") for r in readings]
    if token.lower() in olla_pl2:
        readings=[r.replace("<Pl3>","<Pl2>") for r in readings]
    res=set(readings)
    for r in readings:
        res.add(r.replace("<CC>","<CS>"))
        res.add(r.replace("<CS>","<CC>"))
...
def omorfi_lookup(token):
    ...
    readings=[atRe.sub("",str(r,"utf-8")) for r,score in OmorTransducer.lookup(token.encode("utf-8"))]
...
def is_punct(s):
    ...
    for c in s:
        ...
        if not (cat.startswith("P") or cat=="Sm" or cat=="Sk"): #none of the others? Not a punctuation
            return False
    return True #all characters in the string were punctuation, so the string is punctuation

numRe=re.compile("^[0-9.,:\u2012\u2013\u2014\u2015\u2053~-]+$",re.U) #The unicode chars are various dashes
def is_num(s):
    if numRe.match(s):
        return True
    else:
...
#help function since past.builtins doesn't exist on my system
def own_cmp(x,y):
    return (x>y)-(x<y)

def cand_cmp(xxx_todo_changeme, xxx_todo_changeme1):
    (lemma1,taglist1,dist1) = xxx_todo_changeme
    (lemma2,taglist2,dist2) = xxx_todo_changeme1
    if dist1!=dist2:
        return -own_cmp(dist1,dist2) #the higher, the better
    #Same-distance, make a guess on lemma
    return own_cmp(lemma1.count("|"),lemma2.count("|")) #The lower, the better
...
def omorfi_reading_compatible_with(token,plemma,ptaglist):
    ...
    if not omorfiRawReadings:
        ptaglist[cat2idx["OTHER"]]="UNK" #mark as guess
...
upRe=re.compile(r"^[A-ZÅÄÖ]")
def fill_ortho(token,tagList):
    if upRe.match(token):
        tagList[cat2idx["CASECHANGE"]]="Up"
    else:
...
def hun_possiblepos(token):
    ...
    for r in omorfi_lookup(token):
        try:
            ...
            print("Skipping token:", token.encode("utf-8"), file=sys.stderr)
    return list(res)
...
def hun_possibletags(token):
    ...
    for r in omorfi_lookup(token):
        try:
            ...
            print("Skipping lemma:", lemma.encode("utf-8"), file=sys.stderr)
    return list(res)

def hun_taglist2tagstring(tagList):
    hunposCats=('POS', 'SUBCAT', 'NUM', 'CASE', 'PRS', 'VOICE', 'TENSE', 'MOOD', 'NEG', 'PCP', 'INF', 'CLIT', 'CMP')
    tags=[]
    for cat in hunposCats:
        tag=tagList[cat2idx[cat]]
        if tag!=None:
            tags.append(cat+"_"+tag)
    return "|".join(tags)

def hun_tagstring2taglist(tagString):
    tagList=[None for x in range(len(cat_list))]
    for cat_tag in tagString.split("|"):
        cat,tag=cat_tag.split("_",1)
    ...
    if options.interactive:
        ...
        while True:
            ...
            if options.orig==False:
                readings=omorfi_lookup(str(s,"utf-8"))
            else:
                readings=omorfi_lookup_plain(str(s,"utf-8"))
            if not readings:
                print("--unrecognized--")
                print()
            else:
                for r in readings:
                    if options.orig==False:
                        ...
                        print(lemma.encode("utf-8"),pos.encode("utf-8"),tags.encode("utf-8"))
                    else:
                        print(r.encode("utf-8"))
                print()
    else:
        for token in sys.stdin:
            token=str(token,"utf-8").strip()
            if not token:
                continue
            if options.orig==False:
            ...
            for r in readings:
                if options.orig==False:
                    try:
                        ...
                        if not tags:
                            tags="_"
                        print(("%s\t%s\t%s\t%s"%(token,lemma,pos,tags)).encode("utf-8"))
                    except:
                        pass

                else:
                    print((token+"\t"+r).encode("utf-8"))
            print()
