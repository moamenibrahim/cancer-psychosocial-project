...
ID,FORM,LEMMA,CPOS,POS,FEAT,HEAD,DEPREL,DEPS,MISC=list(range(10))
...
def split(args):
    ...
    for sent in read_conll(args.input):
        ...
        DFS(tree,0,str(count)+u".1",indexes,hier)
        for token in sent:
            token[MISC]=str(indexes[int(token[ID])])
