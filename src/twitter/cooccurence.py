...
def findDistance(w1, w2, tweet):
    ...
    if w1 in words and w2 in words:
        w1_indexes = [index for index,
                      value in enumerate(words) if value == w1]
        w2_indexes = [index for index,
                      value in enumerate(words) if value == w2]
        distances = [abs(item[0] - item[1])
...
def bagOfWords(texts):
    bagsofwords = [collections.Counter(
        re.findall(r'\w+', str(txt))) for txt in texts]
...
with open(occurenceFile) as writefile:
    for line in writefile.readlines():
        ...
        for key in tweet_data['distance'].keys():
            distance = key
...
data_word_total = [data_word[0], data_word[1],
                   data_word[2], data_word[3], data_word[4]]
