    ...
    for line in fileName.readlines():
        ...
        if (any(word.lower() in tweet for word in cancer.mylist)
            ...
            translated = processing.get_translate(
                u"%s" % str(for_translation), tweet_data['lang'])

            if translated:
                dict_result = processing.check_dictionary(
                    u"%s" % str(translated))
                pos = []
