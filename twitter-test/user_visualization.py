...
layout = go.Layout(
    ...
    yaxis=dict(
        title='Percentage of occurence in total',
        titlefont=dict(
            family='Courier New, monospace',
            size=18,
            color='#7f7f7f'
...
data = [go.Bar(
    ...
    y=[ORGANIZATION_count/tweets*100, LOCATION_count/tweets*100, PERSON_count/tweets*100,
        TIME_count/tweets*100, DATE_count/tweets*100, MONEY_count/tweets*100, PERCENT_count/tweets*100]
)]
layout = go.Layout(
    title='Named entity',
    xaxis=dict(
...
fig = go.Figure(data=data, layout=layout)
py.plot(fig, filename='named-entity-percent-bar-user')
